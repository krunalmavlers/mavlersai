...
export default async function Dashboard() {
  ...
  return (
    <div>
        ...
        {cards.map((c) => (
          <Link key={c.label} href={c.href}>
            <Card className="transition-colors hover:border-brand/40">
              <div className="font-display text-[32px] font-bold text-brand">{c.value}</div>
      ...
      <div className="mt-8">
        <Card>
          <h2 className="m-0 mb-2 font-display text-[16px] font-bold text-white">Quick start</h2>
          <ul className="m-0 list-disc pl-5 text-[14px] leading-relaxed text-body-faint">
            <li>Edit page content and sections in <Link href="/admin/pages" className="text-brand">Pages</Link>.</li>
            <li>Write blog posts &amp; case studies in <Link href="/admin/posts" className="text-brand">Insights &amp; Implementations</Link>.</li>
            <li>Add tracking (GA/GTM/Pixel) and SEO defaults in <Link href="/admin/settings" className="text-brand">Settings</Link>.</li>
            <li>Manage header/footer navigation in <Link href="/admin/menus" className="text-brand">Menus</Link>.</li>
          </ul>
        </Card>
      </div>
    </div>
