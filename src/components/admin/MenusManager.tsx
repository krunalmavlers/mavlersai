...
export function MenusManager({ items }: { items: MenuItem[] }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card>
        <h2 className="m-0 mb-4 font-display text-[16px] font-bold text-black">Add menu item</h2>
      ...
      {LOCATIONS.map((loc) => {
        ...
        return (
          <Card key={loc}>
            <h2 className="m-0 mb-3 font-display text-[16px] font-bold capitalize text-black">{loc.replace('_', ' ')}</h2>
...
function MenuRow({ item }: { item: MenuItem }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-2 rounded-[9px] border border-surface-line2 p-2 md:grid-cols-[1.4fr_1.6fr_1fr_0.6fr_auto_auto]">
      ...
      <button onClick={() => start(() => saveMenuItem(f) as any)} className="rounded-[9px] bg-surface-tint2 px-3 py-2 text-[12px] font-bold text-black hover:bg-black/5">
        Save
      </button>
      <button onClick={() => start(() => deleteMenuItem(item.id) as any)} className="rounded-[9px] border border-red-500/40 px-3 py-2 text-[12px] font-semibold text-red-600 hover:bg-red-500/10">
        Del
      </button>
    </div>
  );
