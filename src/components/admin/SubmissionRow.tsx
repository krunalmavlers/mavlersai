    ...
    <Card className="p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          <button onClick={() => setOpen((v) => !v)} className="rounded border border-white/12 px-2.5 py-1 text-[12px] text-body-faint hover:bg-white/5">
            {open ? 'Hide' : 'View'}
          </button>
          <button onClick={() => start(() => updateSubmissionStatus(submission.id, 'read') as any)} className="rounded border border-white/12 px-2.5 py-1 text-[12px] text-body-faint hover:bg-white/5">
            Read
          </button>
          <button onClick={() => start(() => updateSubmissionStatus(submission.id, 'spam') as any)} className="rounded border border-white/12 px-2.5 py-1 text-[12px] text-body-faint hover:bg-white/5">
            Spam
          </button>
          <button
            onClick={() => confirm('Delete submission?') && start(() => deleteSubmission(submission.id) as any)}
            className="rounded border border-red-500/40 px-2.5 py-1 text-[12px] text-red-400 hover:bg-red-500/10"
          >
            Delete
          </button>
        </div>
      ...
      {open && (
        <table className="mt-3 w-full border-t border-white/8 text-left text-[13px]">
          <tbody>
            {Object.entries(data).map(([k, v]) => (
              <tr key={k} className="border-b border-white/6 last:border-0">
