...
export function TaxonomyManager({ categories, tags }: { categories: Category[]; tags: Tag[] }) {
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <Card>
        <h2 className="m-0 mb-4 font-display text-[18px] font-bold text-black">Categories</h2>
        <div className="mb-4 flex flex-col gap-2 rounded-[12px] border border-surface-line2 p-3">
        ...
        <div className="flex flex-col gap-1.5">
          {categories.map((c) => (
            <div key={c.id} className="flex items-center gap-2 rounded-[9px] border border-surface-line2 px-3 py-2 text-[13px]">
              <span className="font-semibold text-black">{c.name}</span>
              <span className="text-body-dim">/{c.slug}</span>
              <span className="rounded bg-surface-tint2 px-1.5 py-0.5 text-[10px] text-body-dim">{c.taxonomy}</span>
              <button
                onClick={() => start(() => deleteCategory(c.id) as any)}
                className="ml-auto text-[12px] font-semibold text-red-600 hover:text-red-600"
              >
                Delete
              </button>
            </div>
      ...
      <Card>
        <h2 className="m-0 mb-4 font-display text-[18px] font-bold text-black">Tags</h2>
        <div className="mb-4 flex gap-2 rounded-[12px] border border-surface-line2 p-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {tags.map((t) => (
            <span key={t.id} className="flex items-center gap-2 rounded-full border border-surface-line2 px-3 py-1 text-[12.5px] text-body-soft">
              #{t.name}
              <button onClick={() => start(() => deleteTag(t.id) as any)} className="text-red-600 hover:text-red-600">
                ×
              </button>
            </span>
          ))}
