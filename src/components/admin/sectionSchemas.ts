...
export const SECTION_SCHEMAS: Record<string, FieldDesc[]> = {
  hero: [
    ...
    },
    {
      ...
      label: 'Hero backdrop',
      kind: 'select',
      options: ['traces', 'plain'],
      hint: 'Centered layout only. traces = the animated circuit lines (default). plain = nothing behind the copy.',
    },
    {
      key: 'theme',
      ...
      kind: 'select',
      options: ['light', 'dark'],
      hint: 'Applies to the centered layout only. dark puts the copy on the black ground with the circuit traces, as used on Book a Call.',
    },
    { key: 'badge', label: 'Badge', kind: 'text' },
    { key: 'heading_html', label: 'Heading (HTML: wrap accent words in <span>…</span>)', kind: 'textarea' },
    {
      key: 'heading_alts',
      label: 'Accent phrase alternatives',
      kind: 'stringList',
      hint: 'Leave empty for a static heading. Add one or more and the accent span cycles through them every 2 seconds, starting with the phrase in the heading above. The heading reserves room for the longest, so keep them similar in length.',
    },
    ...
    { key: 'animated', label: 'Show hero animation (mascot)', kind: 'bool' },
    {
      ...
      kind: 'select',
      options: ['robot', 'ai-pipeline', 'signal-lattice', 'assembly-floor', 'emergence', 'automation-flow', 'validation-loop'],
      hint: 'Fills the space beside the copy on large screens. robot = homepage mascot · ai-pipeline = 3D retrieval → LLM → outcomes scene · signal-lattice / assembly-floor / emergence = the three 9s-loop concept animations · automation-flow = source systems feeding an orchestrator and fanning out to tools · validation-loop = a build/test/learn orbit around a product card. A hero image or background video overrides it.',
    },
    {
      key: 'breadcrumb',
      ...
      itemFields: [
        ...
        { key: 'href', label: 'Link (blank = current page)', kind: 'text' },
      ],
    },
    { key: 'image', label: 'Hero image URL', kind: 'text', hint: 'Shown beside the copy on large screens, instead of the animation.' },
  ...
  connect_grid: [
    {
      ...
      kind: 'select',
      options: ['dark', 'light'],
      hint: 'dark = the black band used on the homepage (default). light = white tiles on the page background.',
    },
    eyebrow,
    heading,
    { key: 'subhead', label: 'Subhead', kind: 'textarea' },
  ...
  services_detail: [
    ...
    heading,
    {
      ...
      itemFields: [
        ...
        { key: 'cross_link', label: 'Cross-link to a related service', kind: 'cta' },
      ],
    },
  ...
  service_categories: [
    ...
    { key: 'subhead', label: 'Subhead', kind: 'textarea' },
    {
      ...
      kind: 'objectList',
      itemFields: [
        { key: 'n', label: 'Number (e.g. 01)', kind: 'text' },
        { key: 'tag', label: 'Corner tag (e.g. Build)', kind: 'text' },
        { key: 'title', label: 'Title', kind: 'text' },
        { key: 'kicker', label: 'Kicker line', kind: 'text' },
        { key: 'body', label: 'Body', kind: 'textarea' },
        { key: 'chips', label: 'Chips', kind: 'stringList' },
        { key: 'href', label: 'Links to', kind: 'text' },
        { key: 'cta_label', label: 'Button label', kind: 'text' },
        { key: 'icon', label: 'Icon name (optional)', kind: 'text' },
      ],
    },
  ],
  cta_band: [
  ...
  rich_text: [
    {
      ...
      label: 'Layout',
      kind: 'select',
      options: ['default', 'aside'],
      hint: 'default = one narrow column, right for legal pages and short notes. aside = the heading keeps a column of its own and stays put while the prose scrolls beside it. Better when several text sections run back to back.',
    },
    eyebrow,
    heading,
